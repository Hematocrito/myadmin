...
export default class FormFeed extends PureComponent<IProps> {
  ...
  async submit(formValues: any) {
    ...
    if (this.teaserId) {
      payload.teaserId = this.teaserId;
    }
    if (this.thumbnailId) {
      payload.thumbnailId = this.thumbnailId;
    }
    ...
    if (addPoll && pollList.length < 2) {
      ...
      payload.pollIds = this.pollIds;
      payload.pollExpiredAt = expiredPollAt;
      this.onsubmit(feed, payload);
    } else {
      await this.setState({ uploading: true });
      this.onsubmit(feed, payload);
    }
  ...
  render() {
    ...
    const { feed } = this.props;
    const {
      uploading, fileIds, isSale, pollList, type,
      ...
      <div className="feed-form">
        ...
        >
          <Form.Item
            ...
            rules={[
              { required: true, message: 'Please select a performer!' }]}
          >
